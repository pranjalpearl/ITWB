...
//         mutationFn: () => ToursService.fetchBestSellingTour(),
//         onSuccess: (data) => {
//             setTours(data);
//         },
//         onError: (error) => {
...
const BestSelling = () => {
    ...
    const { mutate, isPending, isError, error } = useMutation({
        mutationFn: () => ToursService.fetchBestSellingTour(),
        onSuccess: (data) => {
            // Adjusted to access the 'tours' array inside the response object
            setTours(data.tours || []);
        },
        onError: (error) => {
            console.error("Failed to fetch tours:", error);
    ...
    // Changed from tour.is_hot_selling to tour.isHotSelling
    const hotSellingTours = tours.filter((tour) => tour.isHotSelling);
                    ...
                    <div className="h-full w-full">
                        ...
                        >
                            {hotSellingTours.map((tour) => (
                                <SwiperSlide key={tour.id}>
                                    <div className="relative max-w-lg bg-white border border-gray-200 rounded-2xl shadow-2xl dark:bg-gray-800 dark:border-gray-700 mx-auto my-8 overflow-hidden transform hover:scale-105 transition-transform duration-300">
                                        {/* Using tour.id for link since slug was not present in console data */}
                                        <a href={`/tour/detail/${tour.id}`}>
                                            <img
                                                className="rounded-t-2xl h-[450px] w-full object-cover"
                                                // Changed mapping to tour.coverPhotoUrl
                                                src={
                                                    tour.coverPhotoUrl ||
                                                    "/images/placeholder.jpg"
                                                }
                                                // Changed mapping to tour.tourName
                                                alt={tour.tourName}
                                            />
                                        </a>
                                        <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent"></div>
